      ...
      data.forEach((data, i) => {
        ...
        data.val.forEach((d, j) => {
          ...
          bodyElts.push(
              <tr key={j}>
                {leftCol}
                <td key="1" x={x} y={y} style={style}><img width={width} height={height} src={src}/></td>
              </tr>);
        });
        key++;
        elts.push(<table key={key} style={{marginBottom: "0", marginTop: "10", background: "#f5f5f5"}} width="100%">

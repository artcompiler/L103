...
var transform = function () {
  ...
  function desmos(node, options, resume) {
    visit(node.elts[0], options, function (err, val1) {
      visit(node.elts[1], options, function (err, val2) {
        var values = [];
        val2.values.forEach(function (v) {
  ...
  function lambda(node, options, resume) {
    ...
    visit(node.elts[0], options, function (err0, params) {
      ...
      visit(node.elts[1], options, function (err, val) {
        ...
        var lexicon = env.lexicon;
        Object.keys(lexicon).forEach(function (n) {
...
var render = function () {
  if (MATHJAX) {
    var _tex2SVG = function _tex2SVG(str, resume) {
      try {
        mjAPI.typeset({
          math: str,
          format: "TeX",
    ...
    var _escapeXML = function _escapeXML(str) {
  ...
  function getLaTeX(str, hasText) {
    ...
    startMath.forEach(function (v, i) {
      ...
      if (i === 0) {
        ...
        var parts = ["", ""];
        var n = 0;
        done: for (var _i = 0; _i < v.length; _i++) {
          // Look for closing }.
          if (v[_i] === "{") {
            parts[0] += v[_i];
            n++;
            ...
            if (n > 0) {
              ...
              parts[0] = "\\left[\\left[" + parts[0] + "\\right]\\right]";
              parts[1] += v.substring(_i + 1);
              break done;
            }
          } else {
            parts[0] += v[_i];
    ...
    startMath.forEach(function (v, i) {
      ...
      if (i === 0) {
        ...
        var parts = ["", ""];
        var n = 0;
        done: for (var _i2 = 0; _i2 < v.length; _i2++) {
          // Look for closing }
          if (v[_i2] === "{") {
            parts[0] += v[_i2];
            n++;
  ...
  function getMathQuill(str) {
    ...
    startMath.forEach(function (v, i) {
      ...
      if (i === 0) {
        ...
        var parts = ["", ""];
        var n = 0;
        done: for (var _i3 = 0; _i3 < v.length; _i3++) {
          // Look for closing }.
          if (v[_i3] === "{") {
            parts[0] += v[_i3];
            n++;
  ...
  function getPlainText(str) {
    ...
    startMath.forEach(function (v, i) {
      if (i === 0) {
        ...
        var parts = ["", ""];
        var n = 0;
        done: for (var _i4 = 0; _i4 < v.length; _i4++) {
          // Look for closing }.
          if (v[_i4] === "{") {
            parts[0] += v[_i4];
            n++;
  ...
  function render(val, options, resume) {
    ...
    var latex = val.latex;
    var type = val.type || "formula";
    var subtype = val.subtype || undefined;
    var title = val.title;
    var index = val.index;
    var notes = val.notes;
    var origContext = val.context;
    var origTemplate = val.template;
    var values = val.values;
    var errs = [];
    var vals = [];
    var genList = [].concat(val.gen);
    var dynaData = [];
    var isFirst = true;
    mapList(genList, function (v, resume) {
      // TODO if user context or template exists, use it.
      var context = v.context || "{stimulus}";
      var template = v.template || "{response}";
      ...
      if (template) {
        ...
        var word = void 0;
        keys.forEach(function (k, i) {
          var val = v[k] || (word = findWord(options, k)) && word.val;
        ...
        startMath.forEach(function (v, i) {
          ...
          if (i === 0) {
            ...
            var level = 0;
            var eraseClosing = 0;
            done: for (var _i5 = 0; _i5 < v.length; _i5++) {
              // Look for closing }
              if (v[_i5] === "[") {
                level++;
                if (v[_i5 - 1] === "$") {
      ...
      if (context) {
        ...
        var _startMath = cntx.split("{{");
        var _outStr = "";
        var _offset = 0;
        _startMath.forEach(function (v, i) {
          // Even indexes are text, odd have LaTeX prefixes.
          if (i === 0) {
            _outStr += nontrivial(v) ? v : "";
          } else {
            var parts = ["", ""];
            var level = 0;
            var eraseClosing = 0;
            done: for (var _i6 = 0; _i6 < v.length; _i6++) {
              // Look for closing }
              if (v[_i6] === "{") {
                level++;
                if (v[_i6 - 1] === "$") {
            ...
            _outStr += nontrivial(parts[0]) ? "${" + parts[0] + "}" : " ";
            _outStr += nontrivial(parts[1]) ? parts[1] : "";
            _offset++;
          }
        });
        cntx = _outStr;

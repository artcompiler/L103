...
var render = function () {
  ...
  function render(val, resume) {
    ...
    var index = val.index;
    var notes = val.notes;
    var context = val.context || "{stimulus}";
    ...
    mapList(genList, function (v, resume) {
      ...
      if (template) {
        ...
        keys.forEach(function (k, i) {
          ...
          _tmpl = _tmpl.replace(new RegExp("{" + k + "}", "g"), v[k]);
          _tmpl = _tmpl.replace(new RegExp("==" + k + "}", "g"), v[k] + "}");
          _tmpl = _tmpl.replace(new RegExp("\\[\\[" + k + "\\]\\]", "g"), v[k]);
        });
        // Get the right order.
      ...
      if (context) {
        ...
        _keys.forEach(function (k, i) {
          _cntx = _cntx.replace(new RegExp("{" + k + "}", "g"), "${" + v[k] + "}");
          _cntx = _cntx.replace(new RegExp("==" + k + "}}", "g"), v[k] + "}");
          _cntx = _cntx.replace(new RegExp("\\[\\[" + k + "\\]\\]", "g"), v[k]);
        });

        var startMath = _cntx.split("{{");
        var outStr = "";
        var offset = 0;
        startMath.forEach(function (v, i) {
          // Even indexes are text, odd have LaTeX prefixes.

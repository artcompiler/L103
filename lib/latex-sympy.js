...
options.words=_rules2.rules.words;options.rules=_rules2.rules.rules;options.types=_rules2.rules.types;}var spec={method:"translate",options:options};var evaluator=makeEvaluator(spec);evaluator.evaluate(solution,resume);}function makeEvaluator(spec){var valueNode=void 0;var method=spec.method;var value=spec.value;var options=_model.Model.options=spec.options;var pendingError=void 0;try{_assert.Assert.setLocation("spec");validateOptions(options);_model.Model.pushEnv(env);valueNode=value!=undefined?_model.Model.create(value,"spec"):undefined;_model.Model.popEnv();}catch(e){console.log(e.stack);pendingError=e;}var evaluate=function evaluate(solution,resume){try{if(pendingError){throw pendingError;}_assert.Assert.setLocation("user");assert(solution!=undefined,message(3002));_model.Model.pushEnv(env);var solutionNode=_model.Model.create(solution,"user");assert(solutionNode,message(3008,["invalid input"]));_assert.Assert.setLocation("spec");var result=void 0;switch(method){case"translate":result=solutionNode.translate();break;default:assert(false,message(3004,[method]));break;}_model.Model.popEnv();resume([],result);}catch(e){console.log(e.stack);var _message=e.message;resume([{result:null,errorCode:parseErrorCode(_message),message:parseMessage(_message),stack:e.stack,location:e.location,model:null,// Unused, for now.

...
var transform = function () {
  ...
  function expandSpec(node, options, resume) {
    visit(node.elts[0], options, function (err1, val1) {
      console.log("expandSpec() val1=" + JSON.stringify(val1));
      var data = [];
      if (val1.params) {
        data.push(Object.keys(val1.params));
      }
      data = data.concat(generateDataFromSpec(val1.values));
      console.log("expandSpec() data=" + JSON.stringify(data));
      val1.values = data;
      resume([], val1);
    });
    ...
    function generateDataFromSpec(spec) {
      console.log("generateDataFromSpec() spec=" + JSON.stringify(spec));
      var table = expand(spec);
      var data = [];

...
var transform = function () {
  ...
  function factor(node, options, resume) {
    ...
    visit(node.elts[0], options, function (err, val) {
      ...
      errs = errs.concat(err);
      mapList(lst, function (v, resume) {
        console.log("factor() v=" + JSON.stringify(v));
        var obj = {
          func: "eval",
  ...
  function expand(node, options, resume) {
    ...
    visit(node.elts[0], options, function (err, val) {
      errs = errs.concat(err);
      var lst = [].concat(val);
      console.log("expand() lst=" + JSON.stringify(lst));
      mapList(lst, function (v, resume) {
        texToSympy(v, function (err, v) {
          console.log("expand() v=" + JSON.stringify(v));
          var obj = {
            func: "eval",
            expr: "(lambda(x,y,z):expand(" + v + "))(symbols('x y z'))"
          };
          getSympy("/api/v1/eval", obj, function (err, data) {
            console.log("expand() data=" + JSON.stringify(data));
            if (err && err.length) {
              errs = errs.concat(error(err, node.elts[0]));
            }
            resume(errs, v);
  ...
  function render(val, resume) {
    ...
    mapList(lst, function (v, resume) {
      sympyToLaTeX(v, function (err, tex) {
        ...
        //        console.log("render() vals=" + JSON.stringify(vals));
      });
    }, resume);

    // val.forEach(v => {
    //   sympyToLaTeX(v, (err, tex) => {
    //     console.log("render() tex=" + tex);
    //     tex2SVG(tex, function (err, svg) {
    //       if (err) {
    //         errs.push(err);
    //       } else {
